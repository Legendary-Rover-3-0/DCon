...
#define START_RISING_TIME	(10.0f)
#define SET	 				(1u)
#define RESET				(0u)
#define HTIM_PWM_TODO		(NULL) // tu był &htim4
#define HTIM_ENCODER_TODO	(NULL) // tu był &htim3
#define STOP_PIN			(0u) // jaki to pin
#define STOP_GPIO_PORT		(0u) // jaki to port
#define KIERUNEK_PIN		(0u)	// jaki to pin
#define KIERUNEK_GPIO_PORT	(0u) // jaki to port
#define PRACA_GPIO_PORT		(0u) // jaki to port
#define PRACA_PIN			(0u) // jaki to pin
...
void InitFunction(WHEEL_Type* FR)          //,WHEEL_Type* FL,WHEEL_Type* BR,WHEEL_Type* BL)//funkcja incalizujaca 4 obiekty
{
	...
	FR->timer_data._htim_PWM = HTIM_PWM_TODO;
	FR->timer_data._channel_PWM=TIM_CHANNEL_1;

	FR->timer_data._htim_ENCODER = HTIM_ENCODER_TODO;
	...
	FR->ramp_data.RisingTime = START_RISING_TIME;
...
void VelocityCalculation(WHEEL_Type* FR,WHEEL_Type* FL,WHEEL_Type* BR,WHEEL_Type* BL,int speed, int radius) //ta funkcja oblicza jaką predkosc ma mieć kazde kolo
{
	if((FR->MOTOR_STATE==DRIVE)&&(FL->MOTOR_STATE==DRIVE)&&(BR->MOTOR_STATE==DRIVE)&&(BL->MOTOR_STATE==DRIVE))//sprawdzam czy kazde kolo jest w stanie drive
	{
		if((FR->MOVE_STATE==FL->MOVE_STATE)&&(BR->MOVE_STATE==BL->MOVE_STATE)&&(FL->MOVE_STATE==BR->MOVE_STATE))//sprawdzam czy kazde kolo jest w tym samym trybie ruchu
		{
			...
			{
			case IDLE_2:
				FR->set_velocity = 0.0f;                  //w trybie nieustalonym predkosc wynosi 0
				FL->set_velocity=0;
				BR->set_velocity=0;
				BL->set_velocity=0;
				break;
...
#define P_VALUE (0.267f) 
#define I_VALUE (14.1f)
#define D_VALUE (0.0f)
